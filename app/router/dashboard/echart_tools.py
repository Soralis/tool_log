            ...
            for machine in ordered_records:
                for channel in ordered_records[machine]:
                    ...
                    series.append({
                        ...
                        "areaStyle": { "color": "rgba(75,192,192,0.5)" },
                    })
...
async def send_tool_data(websocket: WebSocket, db: Session):
    ...
    try:
        ...
        graphs = await get_tool_graphs(db, latest_start_date, latest_end_date, selected_operations, selected_products)
...
async def tools(request: Request, db: Session = Depends(get_session)):
    ...
    return templates.TemplateResponse(
        "dashboard/echarts_tools.html.j2",  # Updated template path
        {
            "request": request,
            "graphs": graphs
        }
...
@router.get("/api/echart_tool/{tool_id}/details")
...
@router.websocket("/ws/echart_tools")
